...
def bfs(loc):
    global ans_visited, ans
    d = [(0,-1),(1,0),(0,1),(-1,0)]
    for i in range(len(arr_back)):
        q.append((r[loc],c[loc]))
        visited = [[0]*n for _ in range(n)]
        visited[r[loc]][c[loc]] = 1
        while q:
            y,x = q.popleft()
            if ans_visited[y][x] == 0 and not [y,x] in arr_back[i]:
                ans_visited[y][x] = 1
                ans +=1
            for j in range(len(d)):
                dy, dx = d[j]
                oy, ox = dy + y, dx+x
                if in_range(oy,ox) and (grid[oy][ox] != 1 or [oy,ox] in arr_back[i]) and visited[oy][ox] ==0:
                    visited[oy][ox] = 1
                    q.append((oy,ox))
